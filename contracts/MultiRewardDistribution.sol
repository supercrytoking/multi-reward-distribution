...
interface IMultiRewardDistribution {
...
contract Ownable is OwnableData {
    ...
    /// @param renounce Allows the `newOwner` to be `address(0)`
    /// if `direct` and `renounce` is True. Has no effect otherwise.
...
contract MultiRewardDistribution is IMultiRewardDistribution, Ownable {
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory _rewards) {
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            _rewards[i].amount = _earned(account, _rewards[i].token, balance, _rewardPerToken(rewardTokens[i]))
                .div(1e12);
        }
        return _rewards;
    }
    ...
    function stakeFor(address _recipient, uint256 amount) public override {
        require(amount > 0, "Cannot stake 0");
        _updateReward(_recipient);
        totalSupply = totalSupply.add(amount);
        balances[_recipient] = balances[_recipient].add(amount);
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, _recipient, amount);
    }
    ...
    function getReward(address[] memory _rewardTokens) public override {
        _updateReward(msg.sender);
        _getReward(_rewardTokens, msg.sender);
    }

    function getRewardFor(address[] memory _rewardTokens, address recipient) external {
        _updateReward(msg.sender);
    ...
    function notifyReward(address _rewardsToken, uint256 reward) external onlyOwner {
        ...
        require(r.periodFinish > 0, "Unknown reward token");
        r.rewardPerTokenStored = _rewardPerToken(_rewardsToken);
    ...
    function setRewardRate(address _rewardsToken, uint256 _rewardRate) external onlyOwner {
        Reward storage r = rewardData[_rewardsToken];
        require(r.periodFinish > 0, "Unknown reward token");
        uint256 rate = r.rewardRate;
        r.rewardPerTokenStored = _rewardPerToken(_rewardsToken);
        r.rewardRate = _rewardRate;
        r.lastUpdateTime = block.timestamp;
        emit RateChanged(_rewardsToken, rate, _rewardRate);
    }
    ...
    event RewardPaid(address indexed from, address indexed to, address indexed rewardsToken, uint256 reward);
    event Recovered(address token, uint256 amount);
    event RateChanged(address token, uint256 oldRate, uint256 newRate);
}